...
const carouselRef = useTemplateRef<InstanceType<typeof Carousel>>('carousel')
...
const {Shift_Ctrl_ArrowLeft, Shift_Ctrl_ArrowRight} = useMagicKeys()
...
watch(Shift_Ctrl_ArrowLeft, v => {
  if (v) carouselRef.value?.scrollPrev()
})

watch(Shift_Ctrl_ArrowRight, v => {
  if (v) carouselRef.value?.scrollNext()
})
</script>

<template>
  <GlassProvider>
    <Carousel ref="carousel" no-key :opts="{watchDrag: false}" @selected="(index) => currentTabIndex = index"
              class="absolute top-0 focus-visible:outline-0">
